...
function Header() {
  return (
    <div className="rounded-2xl border border-[#e5e5e5] bg-gradient-to-br from-[#e5e5e5] to-[#fafafa] p-6 sm:p-8">
      <div className="flex items-start gap-4">
        <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-white text-[#262626] shadow-sm">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-sm leading-relaxed text-[#171717]/70 sm:text-base">
            ...
            <span className="text-[#262626]"> congelados, refrigerados</span> e
      ...
      <div className="rounded-xl border border-[#F2C9B8] bg-[#FBE9E0]/40 p-4">
        <div className="flex gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-[#171717]/75">
    ...
    >
      <ol className="space-y-3">
        {[
          ...
          <li
            key={step.n}
            className="relative rounded-xl border border-[#e5e5e5] bg-white p-4 pl-12"
          >
            <span className="absolute left-4 top-4 inline-flex h-7 w-7 items-center justify-center rounded-full bg-[#262626] text-xs font-semibold text-white">
              {step.n}
            </span>
            <h4 className="text-sm font-semibold text-[#171717]">
              {step.title}
            </h4>
            <p className="mt-1 text-sm leading-relaxed text-[#171717]/70">
              {step.body}
            </p>
            {step.cta ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-2 inline-flex items-center gap-1.5 text-xs font-medium text-[#262626] hover:underline"
              >
                {step.cta.label}
      ...
      <ul className="space-y-2">
        {items.map((it) => (
          ...
          >
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold text-[#171717]">{it.label}</p>
              <p className="mt-0.5 text-sm text-[#171717]/65">{it.reason}</p>
            </div>
          </li>
        ))}
      </ul>
...
function Section({
  ...
  return (
    <section className="rounded-2xl border border-[#e5e5e5] bg-white p-5 shadow-[0_8px_24px_-16px_rgba(0,0,0,0.18)] sm:p-7">
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-start gap-3">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#e5e5e5] text-[#262626]">
          ...
          <div className="min-w-0">
            ...
            <h2 className="mt-0.5 text-base font-semibold leading-tight text-[#171717] sm:text-lg">
              {title}
            </h2>
            {subtitle ? (
        ...
        {accentBadge ? (
          <span className="self-start rounded-full bg-[#262626] px-3 py-1 text-xs font-medium text-white sm:self-auto">
            {accentBadge}
          </span>
        ) : null}
      </div>
...
function SpecsTable({ rows }: { rows: [string, string][] }) {
  return (
    <dl className="grid gap-x-6 gap-y-2 rounded-xl border border-[#e5e5e5] bg-[#fafafa] p-4 text-sm sm:grid-cols-[max-content_1fr]">
      {rows.map(([k, v]) => (
        <div key={k} className="contents sm:[&>dt]:py-1 sm:[&>dd]:py-1">
          <dt className="font-medium text-[#171717]/55">{k}</dt>
          <dd className="text-[#171717]">{v}</dd>
        </div>
      ))}
    </dl>
  );
  ...
  return (
    <ul className="space-y-1.5">
      {items.map(([ok, text]) => (
        <li
          key={text}
          className="flex items-start gap-2 text-sm text-[#171717]/80"
        >
          {ok ? (
            <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[#262626]" />
          ) : (
...
function BuyButtons({ label, links }: { label: string; links: BuyLink[] }) {
  return (
    <div>
      <p className="mb-2 text-xs font-medium uppercase tracking-wider text-[#171717]/50">
        {label}
      </p>
      <div className="flex flex-wrap gap-2">
        {links.map((l) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 rounded-full border border-[#262626]/20 bg-white px-4 py-2 text-xs font-medium text-[#262626] transition hover:border-[#262626]/40 hover:bg-[#e5e5e5]"
          >
