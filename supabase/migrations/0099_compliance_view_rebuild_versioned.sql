-- =====================================================================
-- 0099_compliance_view_rebuild_versioned.sql
--
-- Elimina o drift da 0039: aquele arquivo era um stub ("corpo identico
-- ao aplicado via MCP") — o corpo real da company_compliance_v so
-- existia no banco, sem garantia versionada de que a recriacao manteve
-- `security_invoker = true`. Sem security_invoker, a view roda como o
-- OWNER e o GRANT a authenticated exporia as metricas de TODAS as
-- empresas (cross-tenant) para qualquer usuario logado.
--
-- Esta migration recria a view de forma deterministica:
--   - corpo da 0032 + filtros de soft-delete prometidos pela 0039
--     (companies/audits/documents/manipulators com deleted_at is null);
--   - `security_invoker = true` explicito (RLS das tabelas-base manda);
--   - GRANT apenas para authenticated (anon foi revogado na 0072 e o
--     DROP abaixo apagaria esse estado — nao re-conceder).
--
-- Colunas preservadas (get_company_seal / dashboard dependem delas).
-- =====================================================================

drop view if exists public.company_compliance_v;

create view public.company_compliance_v
...
select
  ...
  coalesce(pc.has_pest_service_30d, false) as has_pest_service_active,
  coalesce(pc.has_pest_registered, false) as has_pest_service_registered,
  pc.last_pest_at as last_pest_at,
  pc.next_pest_due_at as next_pest_due_at
...
left join lateral (
  select
    max(performed_at) as last_pest_at,
    max(next_due_at) as next_pest_due_at,
    coalesce(
      max(case when next_due_at >= current_date then 1 else 0 end) = 1,
      false
    ) as has_pest_service_30d,
    count(*) > 0 as has_pest_registered
  from public.pest_control_services pcs
  where pcs.company_id = c.id
) pc on true
where c.deleted_at is null;

-- Sem anon (0072); RLS das tabelas-base decide o que cada um ve.
grant select on public.company_compliance_v to authenticated;
